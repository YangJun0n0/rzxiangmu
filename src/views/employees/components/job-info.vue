...
import { getEmployeesJobsAPI, saveEmployeesJobsAPI } from '@/api/employees'
export default {
  ...
  methods: {
    async loadEmployeesJobs() {
      try {
        this.formData = await getEmployeesJobsAPI(this.userId)
      } catch (error) {
        console.log(error)
      }
    },
    async saveEmployeesJobs() {
      try {
        await saveEmployeesJobsAPI(this.formData)
